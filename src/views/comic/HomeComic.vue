<template>
  <div class="home-comic">
    <scroll :top="0" :bottom="48" @onScroll="onScroll" ref="scroll">
      <van-swipe :autoplay="3000" indicator-color="#ff9a6a">
        <van-swipe-item v-for="image in banner" :key="image">
          <img
            v-lazy="image"
            class="banner"
            @click.stop.prevent="selectComic(recommendEveryDay)"
          />
        </van-swipe-item>
      </van-swipe>
      <img
        src="../../assets/image/comic_block_header.png"
        class="block-header"
      />
      <div class="block">
        <comic-item
      ...
      <img
        src="../../assets/image/comic_recommend_everyday_header.png"
        class="block-header"
      />
      <div
        class="recommend-everyday"
        @click.stop.prevent="selectComic(recommendEveryDay)"
      >
      ...
      <img
        src="../../assets/image/comic_update_today_header.png"
        class="block-header"
      />
      <div class="update-today">
        <img
          src="../../assets/image/comic_update_today_left.png"
          class="update-today-left"
        />
        <div class="update-today-right">
          <div
            class="update-today-right-item"
            v-for="item in updateTodayList"
            :key="item.title"
          >
            <img v-lazy="item.cover" />
            <div
              class="update-today-right-text"
              @click.stop.prevent="selectComic(item)"
            >
...
import HomeComicData from '../../mock/home_comic.json'
...
export default {
  ...
  created () {
    ...
    this.http
      ...
      .catch(error => {
        console.log(error)
        var response = HomeComicData.data
        this.banner = response.banner
        this.blockList = response.blockList
        this.recommendEveryDay = response.recommendEveryDay
        this.updateTodayList = response.updateTodayList
